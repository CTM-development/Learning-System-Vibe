...
func (s *Server) Handler(dist fs.FS) http.Handler {
	...
	mux.HandleFunc("PATCH /api/questions/{id}", s.handlePatchQuestion)
	...
	mux.HandleFunc("POST /api/reviews/undo", s.handleUndoReview)
	mux.HandleFunc("POST /api/cards/{id}/bury", s.handleBuryCard)
